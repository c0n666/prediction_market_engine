...
def plot_equity_curve(
    ...
    *,
    ...
    """
    Equity curve + non-zero trade PnL waterfall + max drawdown subplot.
    ...
    fig = make_subplots(
        ...
        subplot_titles=("Portfolio Equity", "Trade PnL (non-zero)", "Drawdown (%)"),
    )
    ...
    if trades_df is None or trades_df.empty:
        fig.update_layout(
            title=title,
            showlegend=False,
            margin=dict(t=110, b=50, l=50, r=50),
            title_pad=dict(b=20),
        )
        return fig
    ...
    pnl = (
        df[pnl_col].to_numpy(dtype=float)
        if pnl_col in df.columns
        else np.diff(equity, prepend=float(equity[0]))
    )
    if "drawdown" in df.columns:
        drawdown = np.clip(df["drawdown"].to_numpy(dtype=float), 0.0, 1.0)
        max_dd_pct = (
            float(np.clip(np.max(drawdown) * 100.0, 0.0, 100.0)) if len(drawdown) else 0.0
        )
    else:
    ...
    # --- Trade PnL: drop zero-PnL periods so bars stay readable ---
    nonzero_mask = np.abs(pnl) > 1e-12
    if np.any(nonzero_mask):
        trade_idx = np.flatnonzero(nonzero_mask)
        pnl_active = pnl[nonzero_mask]
        x_active = np.arange(1, len(pnl_active) + 1)
        fig.add_trace(
            go.Waterfall(
                x=x_active,
                y=pnl_active,
                measure=["relative"] * len(pnl_active),
                name="Trade PnL",
                increasing={"marker": {"color": "#2e7d32"}},
                decreasing={"marker": {"color": "#c62828"}},
                connector={"line": {"color": "#b0bec5", "width": 1}},
                hovertemplate=(
                    "Active trade %{x}<br>Original #=%{customdata}"
                    "<br>PnL=%{y:.4f}<extra></extra>"
                ),
                customdata=trade_idx,
            ),
            row=2,
            col=1,
        )
    ...
    # --- Drawdown (%): negative series, Y scaled to actual depth ---
    drawdown_pct = drawdown * 100.0
    dd_plot = -drawdown_pct
    fig.add_trace(
        go.Scatter(
            x=x,
            y=dd_plot,
            ...
            hovertemplate="Trade %{x}<br>DD=%{customdata:.4f}%<extra></extra>",
    ...
    fig.update_layout(
        title={
            "text": f"{title} | Max DD: {max_dd_pct:.4f}%",
            "y": 0.98,
            "x": 0.0,
            "xanchor": "left",
            "yanchor": "top",
    ...
    # Push subplot titles upward so they clear axis labels of the panel above
    for annotation in fig.layout.annotations:
        annotation.font.size = 12
        annotation.yshift = 14

    # Capital axis: exact dollar ticks (avoid SI "10.003k")
    fig.update_yaxes(
        title_text="Capital ($)",
        tickformat="$,.2f",
        exponentformat="none",
    ...
    # Dynamic drawdown range — tiny DD (e.g. -0.005%) must remain visible
    if len(dd_plot):
        y_min = float(np.min(dd_plot))
        if y_min >= -1e-12:
            dd_range = [-0.01, 0.002]
        else:
            lower = y_min * 1.2
            upper = max(0.0, abs(y_min) * 0.05)
            dd_range = [lower, upper]
        fig.update_yaxes(
            title_text="Drawdown (%)",
            range=dd_range,
            autorange=False,
    ...
    # Keep X titles, with enough gap from the next subplot title via vertical_spacing
    fig.update_xaxes(title_text="Trade #", title_standoff=8, row=1, col=1)
    fig.update_xaxes(title_text="Active trade #", title_standoff=8, row=2, col=1)
    fig.update_xaxes(title_text="Trade #", title_standoff=8, row=3, col=1)
    return fig
